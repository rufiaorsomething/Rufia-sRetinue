...
extern MY_HIGHP_OR_MEDIUMP vec2 ripped_debuff;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	//Ripped Rendering Start

	float opacity = 1.0;
	//Recenter
	if (uv.x - 0.25 < 0) { opacity = 0; }
	...
	tex = Texel(texture, new_texture_coords);
	...
	if (ripped_debuff.g > 0.0 || ripped_debuff.g < 0.0) {
		width = 0.1;
	}
	bool test = false;
